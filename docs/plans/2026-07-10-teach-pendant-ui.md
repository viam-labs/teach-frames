...
3. ~~`@viamrobotics/sdk` version / exports~~ — **RESOLVED** (Task 8 install): peer is actually `>=0.57`; using `@viamrobotics/sdk@0.57.0` with `svelte@5.56.4`. Confirmed exports: `PoseTrackerClient` (`doCommand(command, callOptions?): Promise<JsonValue>`, ctor `(client: RobotClient, name, options?)`), `ArmClient`, `DialConf`/`DialDirectConf`/`DialWebRTCConf`, `Credentials`, `createRobotClient`, `RobotClient`. Note: `svelte-check` needed a `src/vite-env.d.ts` `*.svelte` module shim (added). npm audit flags 5 transitive dev-dep vulns — review before shipping.
